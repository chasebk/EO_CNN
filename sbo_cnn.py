#!/usr/bin/env python
# ------------------------------------------------------------------------------------------------------%
# Created by "Thieu Nguyen" at 18:47, 29/03/2020                                                        %
#                                                                                                       %
...
from models.main.hybrid_cnn import SboCnn
from utils.IOUtil import _load_dataset__
from utils.Settings import *
from utils.Settings import sbo_cnn_final as param_grid
...
def train_model(item):
    ...
    paras_name = "hs_{}-ac_{}--fs_{}-ks_{}-ps_{}--ep_{}-ps_{}-alpha_{}-pm_{}-z_{}".format(item["hidden_sizes"], item["activations"], item["filters_size"],
                       item["kernel_size"], item["pool_size"], item["epoch"], item["pop_size"], item["alpha"], item["pm"], item["z"])
    root_hybrid_paras = {
    ...
    sbo_paras = {"epoch": item["epoch"], "pop_size": item["pop_size"], "alpha": item["alpha"], "pm": item["pm"], "z": item["z"]}

    md = SboCnn(root_base_paras=root_base_paras, root_hybrid_paras=root_hybrid_paras, cnn_paras=cnn_paras, sbo_paras=sbo_paras)
    md._running__()
